...
export default function getImages(feature, result) {
  const type = result ? 'webp' : 'jpg';
  // const boyType = result ? 'webp' : 'png';

  // const images = {};
  // const boyType = type === 'jpg' ? 'png' : 'webp';
  ...
  //   }-${imageWidth}.${type}`;
  //   const blurredSource = `/chapter-${number}/blurred/chapter-${number}-ink-blur-0x15-160.${type}`;
  ...
  //   if (path.includes('boy') && !path.includes('blur') && boyType === 'png' && imageWidth >= 2880 && imageWidth <= 3000) {
  ...
  //       source = `/${path}/${filePrefix}-ink-blur-0x15-160.${path.includes('boy') ? `${boyType}` : `${type}`}`;
  //     } else {
  //       source = `/${path}/${filePrefix}-imc-main-101419-${
  //         !path.includes('boy')
  //           ? imageWidth < 2880 ? 'q90-' : 'q50-'
  //           : ''
  //       }${imageWidth}.${path.includes('boy') ? `${boyType}` : `${type}`}`;
  //     }
  //   }
  ...
  // [
  //   `/business-card/teen-fairy-img-q90-640-4x.${type}`,
  //   `/not-found/jinni-img-q90-1240-4x.${type}`
  // ].forEach((src, idx) => {
  //   const image = new Image();
  //   image.src = src;
  //   images[
