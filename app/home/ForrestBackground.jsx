...
export default styled.img`
  ...
  opacity: ${p => (!p.homePageLoaded && p.loadLevel < 2) || p.inCity ? '0' : '1'};
  transform: ${p => p.inCity ? 'scale(1.35)' : 'scale(1)'};
  transform-origin: 50% 5%;
  // Transition doesn't run when opacit is 0 b/c we're in NYC — no need...
  transition: ${p => (!p.homePageLoaded && p.loadLevel < 2) || p.spellLevel > 0 && 'transform 1.75s, opacity 1.35s cubic-bezier(0.77, 0, 0.175, 1)'};
